...
import {
	getAccessCodesBySerialNumber,
	getIPAddressesBySerialNumber
} from '../lib/bambu';
...
type Result = { serialNumber: string } & (
	| { ok: true }
...
module.exports = {
	...
	action: async (client: Discord.Client) => {
		if (!ENABLE_CHECK) {
			console.log(
				'Bambu printer access code check is disabled. Skipping.'
			);
			return;
		}
		...
		const [serialNumbersToIpAddresses, serialNumbersToAccessCodes] =
			await Promise.all([
				getIPAddressesBySerialNumber(),
				getAccessCodesBySerialNumber()
			]);

		function verifyPrinterAccessCode(
			...
			return new Promise((res) => {
				...
				const { ip } = entry;
				const accessCode =
					serialNumbersToAccessCodes[printerSerialNumber];
		...
		const results = await Promise.all(
			Object.keys(serialNumbersToIpAddresses).map(verifyPrinterAccessCode)
		);
		...
		for (const result of results) {
			if (!result.ok) {
				...
				if (result.error.type === 'ACCESS_CODE_ERROR') {
					await channel.send(
						`Printer ${printer.name} has an incorrect access code in the printer profile repo. Please update the access code in the repo.`
					);
				} else {
					await channel.send(
						`Printer ${
							printer.name
						} error when checking access code.\n\`\`\`json\n${JSON.stringify(
							result,
							null,
							2
						)}\n\`\`\``
